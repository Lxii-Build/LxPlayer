...
- **播放页**：流光云背景与流光云布局（`player_fluid_cloud_*`），当前歌词三行窗口
  ——当前行更大更粗、其余 50% 透明，窗口随播放进度滚动；另有队列面板与歌曲百科面板
- **歌词**：解析见 `lib/utils/lyric_parser.dart`，含 YRC 逐字格式；
  另有 Android 悬浮歌词与桌面歌词窗
- **下载与缓存**、**系统媒体通知**、**桌面小部件**
