...
class LxPlayerConfig {
  ...
  factory LxPlayerConfig.fromJson(Map<String, dynamic> json) {
    return LxPlayerConfig(
...
/// 
/// 用于解析和解密 .lxcfg 配置文件
class LxConfigService {
  // 单例模式
  static final LxConfigService _instance = LxConfigService._internal();
  factory LxConfigService() => _instance;
  LxConfigService._internal();

  // 加密密钥（32字节 = 256位）- 必须与后端密钥一致
  static final Uint8List _encryptionKey = Uint8List.fromList(
    'LxPlayer2024SecretKey12345678'.codeUnits,
  );

  // 魔数标识
  static const List<int> _magicNumber = [0x4C, 0x58, 0x50, 0x4C]; // "LXPL"
  ...
  /// 解密 .lxcfg 配置文件
  /// 
  /// [data] - .lxcfg 文件的原始字节数据
  /// 
  /// 返回解密后的配置，如果解密失败则返回 null
  LxPlayerConfig? decrypt(Uint8List data) {
    try {
      // 验证文件格式
      ...
      return LxPlayerConfig.fromJson(json);
    } catch (e) {
      print('LxConfigService: 解密失败 - $e');
      return null;
    }
  }
  ...
  bool _validateFormat(Uint8List data) {
    ...
    if (version != _supportedVersion) {
      print('LxConfigService: 不支持的版本 $version');
      return false;
    }
  ...
  Uint8List? _aesGcmDecrypt(Uint8List ciphertext, Uint8List iv, Uint8List authTag) {
    try {
      ...
      print('LxConfigService: AES-GCM 解密失败 - $e');
      return null;
    }
  }

  /// 检查是否为有效的 .lxcfg 文件（仅检查格式，不解密）
  bool isValidLxPlayerFile(Uint8List data) {
