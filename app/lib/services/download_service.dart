...
class DownloadService extends ChangeNotifier {
  ...
  Future<void> _setDefaultDownloadPath() async {
    try {
      if (Platform.isAndroid) {
        // Android: /storage/emulated/0/Download/LxPlayer
        final downloadsDir = Directory('/storage/emulated/0/Download');
        final appDownloadDir = Directory('${downloadsDir.path}/LxPlayer');
        ...
        // Windows: 用户文档/Music/LxPlayer
        final documentsDir = await getApplicationDocumentsDirectory();
        final musicDir = Directory('${documentsDir.parent.path}\\Music\\LxPlayer');
        ...
        // 其他平台：使用文档目录
        final documentsDir = await getApplicationDocumentsDirectory();
        final appDownloadDir = Directory('${documentsDir.path}/LxPlayer');
  ...
  Future<bool> _downloadFromCache(Track track, String outputPath) async {
    try {
      ...
      final cacheFilePath = '${cacheService.currentCacheDir}/$cacheKey.lxcfg';
  ...
  Uint8List _buildVorbisCommentBlock(String? lyrics, {required String title, required String artist, String? album, bool isLast = false}) {
    ...
    // Vendor string length (4 bytes) + Vendor string ("LxPlayer")
    final vendor = utf8.encode('LxPlayer');
