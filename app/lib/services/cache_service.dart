...
class CacheService extends ChangeNotifier {
  ...
  // 加密密钥（用于简单的异或加密）
  static const String _encryptionKey = 'LxPlayerCacheKey2025';
  ...
  String _getCacheFilePath(String cacheKey) {
    return '${_cacheDir!.path}/$cacheKey.lxcfg';
  }
  ...
  Future<bool> cacheSong(
    ...
    try {
      ...
      final cacheBytes = BytesBuilder();
      
      // 写入元数据长度（4字节，大端序）
      cacheBytes.addByte((metadataLength >> 24) & 0xFF);
      cacheBytes.addByte((metadataLength >> 16) & 0xFF);
      cacheBytes.addByte((metadataLength >> 8) & 0xFF);
      cacheBytes.addByte(metadataLength & 0xFF);
      
      // 写入元数据
      cacheBytes.add(metadataBytes);
      
      // 写入加密的音频数据
      cacheBytes.add(encryptedAudioData);

      // 保存 .lxcfg 文件
      final cacheFilePath = _getCacheFilePath(cacheKey);
      final cacheFile = File(cacheFilePath);
      await cacheFile.writeAsBytes(cacheBytes.toBytes());

      print('🔒 [CacheService] 保存缓存文件: $cacheFilePath');
      print('📊 [CacheService] 文件大小: ${cacheBytes.length} bytes (元数据: $metadataLength bytes)');
  ...
  Future<void> _loadCacheIndex() async {
    try {
      final indexFile = File('${_cacheDir!.path}/cache_index.lxcfg');
