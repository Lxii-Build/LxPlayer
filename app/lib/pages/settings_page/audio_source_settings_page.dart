...
import '../../services/lx_config_service.dart';
...
class _AddAudioSourceDialogState extends State<AddAudioSourceDialog> {
  ...
  /// 导入 .lxcfg 加密配置文件
  Future<void> _importLxConfig() async {
    try {
      // 选择 .lxcfg 文件
      ...
      final result = await FilePicker.platform.pickFiles(
        ...
        allowedExtensions: isMobile ? null : ['lxcfg'],
        allowMultiple: false,
      );
      ...
      // 解密配置
      final config = LxConfigService().decrypt(bytes);
      if (config == null) {
                  ...
                  child: Column(
                    children: [
                      ...
                      FilledButton.icon(
                        ...
                        label: const Text('导入 .lxcfg 配置文件'),
                      ),
                    ],
                  ),
                ),
