...
/// `PlayerLyricsPanel` 的行高模型回归测试。
///
/// 起因：面板原来把容器高度**硬切八等分**（`constraints.maxHeight / 8`）再用
/// `SizedBox(height:)` 死死约束每一行，而开启译文的一行要装「原文 + 间距 + 译文」，
/// 在 280px 可用高度下需要 39.8~45.4px、只分到 35px，必然 `RenderFlex overflowed`
/// —— 用户会看到黄黑条纹，译文被裁掉。
///
...
///
/// 放在 `test/` 而不是 `test_visual/`：视觉 job 是 `continue-on-error`，拦不住回归；
/// 这条契约必须由阻断式的 `flutter test` 来守。
///
/// 面板只读 `PlayerService().themeColorNotifier`（默认 null），不触发平台/网络调用。

/// 与 `PlayerLyricsPanel` 内部的排版常量保持一致。
...
/// 面板为跨平台字体度量舍入预留的冗余（`_lineHeightSlack`）。
...
/// （测试环境不加载 Microsoft YaHei，依赖字体度量的断言会本地过、CI 挂）。
...
void main() {
  group('歌词面板行高模型', () {
    testWidgets('开启译文时，各种面板高度都不再溢出', (WidgetTester tester) async {
      ...
      for (final double height in heights) {
        ...
        expect(
          ...
          isNull,
          reason: '面板高 $height 时开启译文不应有任何渲染异常（含 RenderFlex overflowed）',
        );
      }
    });
    ...
    testWidgets('面板够高时：开译文仍是 8 行，当前行仍在第 4 行', (WidgetTester tester) async {
      ...
      // 520 可用高度均分给 8 行 = 65，远大于内容需要的 45.4。
      for (final _Slot slot in layout) {
